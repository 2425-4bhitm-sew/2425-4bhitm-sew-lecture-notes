...
/**
 * @deprecated since 6.1; use contexthub.module.commerce.recentlyviewed instead.
 */

if (window.CQ_Analytics
    && window.CQ_Analytics.ViewedProducts) {
    CQ_Analytics.ViewedProducts.renderer = function(store, divId) {
        CQ_Analytics.ViewedProducts.internalRenderer(store, divId);
    };

    CQ_Analytics.ViewedProducts.internalRenderer = function(store, divId) {
        var storeDiv = $CQ("#" + divId), i;
        storeDiv.children().remove();

        var div = $CQ("<div></div>").addClass("cq-cc-viewedproducts"),
            title = $CQ("<span></span>").addClass("cq-cc-viewedproducts-title").text(CQ.I18n.getMessage("Recently viewed:"));
        title.appendTo(div);
        for (i = 0; i < store.data.length && i < 7; i++) {
            var product = store.data[i],
                thumbnailUrl = product['image'] ? CQ.shared.HTTP.externalize(product['image']) + ".thumbnail.png" : "";
            $CQ("<img>")
...
if (window.CQ_Analytics
    && window.CQ_Analytics.RelatedProducts) {
    CQ_Analytics.RelatedProducts.renderer = function(store, divId) {
        CQ_Analytics.RelatedProducts.internalRenderer(store, divId);
    };

    CQ_Analytics.RelatedProducts.internalRenderer = function(store, divId) {
        var storeDiv = $CQ("#" + divId);
        storeDiv.children().remove();

        var relatedProducts = {}, i, relationship, relationshipTitle;
        for (i = 0; i < store.data.length; i++) {
            relationship = store.data[i];
            relationshipTitle = relationship['relationshipTitle'];
            if (!relatedProducts[relationshipTitle]) {
                relatedProducts[relationshipTitle] = [];
            }
            relatedProducts[relationshipTitle].push(relationship);
        }

        var div = $CQ("<div></div>").addClass("cq-cc-relatedproducts");
        for (relationshipTitle in relatedProducts) {
            if (!relatedProducts.hasOwnProperty(relationshipTitle)) {
                continue;
            }
            var category = $CQ("<div></div>").addClass("cq-cc-relatedproducts-category"),
                title = $CQ("<span></span>").addClass("cq-cc-relatedproducts-categorytitle").text(relationshipTitle);
            title.appendTo(category);
            for (i = 0; i < relatedProducts[relationshipTitle].length && i < 7; i++) {
                var product = relatedProducts[relationshipTitle][i];
                $CQ("<img>")
                    .attr("title", product['title'])
                    .attr("src", CQ.shared.HTTP.externalize(product['image']))
                    .appendTo(category);
            }
...
if (window.CQ_Analytics
    && window.CQ_Analytics.OrderHistoryMgr) {
    ...
    CQ_Analytics.OrderHistoryMgr.internalRenderer = function(store, divId) {
        ...
        if (!store.data["traits"]) {
            storeDiv.append($CQ("<div></div>").text(CQ.I18n.getMessage("Not on an eCommerce page.")));
            return;
        }

        var div = $CQ("<div></div>");
        var divClear = $CQ("<div></div>").addClass("cq-cc-clear");
        var traits = store.data.traits;
        if (traits) {			
            for (var trait in traits) {
                var traitValue = traits[trait];
				var tra = $CQ("<div></div>").addClass("cq-cc-orderhistory-trait").append(traitValue.name + ":");
                if (!traitValue.data)
                    continue;

                if(traitValue.data instanceof Array) {
					for (var i = 0; i < traitValue.data.length; i++) {
                        var pr = $CQ("<span></span>").addClass("cq-cc-orderhistory-property").append(traitValue.data[i]);
                        tra.append(" ");
                    	tra.append(pr);
                    }                    
                } else if(typeof traitValue.data === 'object') {
                    for (var prop in traitValue.data) {
                      	var pr = $CQ("<span></span>").addClass("cq-cc-orderhistory-property").text(prop + "=" + traitValue.data[prop]);
                      	tra.append(" ");    
					  	tra.append(pr);
                    }
                } else {
                    var pr = $CQ("<span></span>").addClass("cq-cc-orderhistory-property").text(traitValue.data);
					tra.append(" ");    
					tra.append(pr);
                }
                div.append(tra);                
...
if (window.CQ_Analytics
    ...
    CQ_Analytics.CartMgr.internalRenderer = function(store, divId) {
        ...
        if (store.data["vouchers"] && store.data["vouchers"].length > 0) {
            var vouchers = $CQ("<div></div>")
                .addClass("cq-cc-cart-vouchers")
                .attr("title", "/cart/vouchers");
            for (var i = 0; i < store.data["vouchers"].length; i++) {
                var voucher = store.data["vouchers"][i];

                var deleteButton = $CQ("<a></a>").attr("href", "#").attr("data-voucher", voucher["code"]);
                vouchers.append(deleteButton);

                var voucherTag = $CQ("<span></span>")
